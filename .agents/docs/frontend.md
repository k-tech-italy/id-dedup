...
- `workflow/templates/workflow/` — `upload.html`, `ticket_list.html`, `ticket_detail.html`, `_pagination.html` (plain Django template renders; the detail page uses a checkbox form for kept images and a disabled form + closed badge once the ticket is closed; the list page includes `_pagination.html` for page-size/pager controls). The upload page is the one workflow page with client-side JS — inline vanilla JS for drag-drop, the file accumulator, and the "processing…" state — no HTMX or Alpine.
